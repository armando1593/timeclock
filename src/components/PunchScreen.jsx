...
export default function PunchScreen() {
  ...
  useEffect(() => { if (pin.length === 4) setTimeout(tryVerify, 200) }, [pin])
  ...
  async function doPunch() {
    ...
    try {
      ...
      stopCam()
      setTimeout(() => { setEmpleado(null); setMsg(null); setGps(null); setGpsStatus('idle'); setGeoError(null) }, 2800)
    } catch(e) {
      setMsg({ text:'Error al guardar. Intenta de nuevo.', type:'error' })
    } finally { setLoading(false) }
  }

  function cancelPunch() { stopCam(); setEmpleado(null); setPin(''); setMsg(null); setGpsStatus('idle'); setGeoError(null) }
    ...
    <div className="punch-screen">
      ...
      {!empleado ? (
          ...
          <div className="pin-grid">
            {KEYS.map(k => (
              <button key={k} className={`pin-key ${k==='✓'?'pin-confirm':k==='←'?'pin-del':''}`}
                onClick={() => handleKey(k)} disabled={loading}>{k}</button>
            ))}
          </div>
        ...
        <div className="confirm-card">
          ...
          {msg && <div className={`msg msg-${msg.type}`}>{msg.text}</div>}
          <button className={`punch-btn ${isIn?'btn-out':'btn-in'}`} onClick={doPunch} disabled={loading || !!geoError}>
